...
////////////////////////////////////////////////////////////////////////////////////
    ...
    /*
     * The week table mixin is just an extension upon day table mixin. The only thing it does is
     * redefining some methods and replaces duration from `1 days` to `1 weeks`.
     * Ideally this should be possible to set outside, but until it's not possible we have to monkey patch it.
     */
    var WeekTableMixin = jQuery.extend(true, {}, DayTableMixin, {

        /*
         * This method is from BasicView. By default is displays week numbers.
         */
        renderIntroHtml: function () {
            return '';
        },
        ...
        updateDayTable: function () {
            ...
            while (date.isBefore(this.end)) { // loop each day from start to end
                ...
                date.add(1, 'months'); // <<=== monkey patch here
            }
        ...
        getCellRange: function (row, col) {
            var start = this.getCellDate(row, col);
            var end = start.clone().add(1, 'months'); // <<=== monkey patch here
        ...
        getDateDayIndex: function (date) {
            var dayIndices = this.dayIndices;
            var dayOffset = date.diff(this.start, 'months'); // <<=== monkey patch here
        ...
        sliceRangeByDay: function (range) {
            ...
            var normalRange = this.view.computeDayRange(range); // make whole-day range, considering nextDayThreshold
            var rangeFirst = this.getDateDayIndex(normalRange.start); // inclusive first index
            var rangeLast = this.getDateDayIndex(normalRange.end.clone().subtract(1, 'months')); // inclusive last index  // <<=== monkey patch here
            var segs = [];
            var row;
            var rowFirst, rowLast; // inclusive day-index range for current row
            var i;
        ...
        getDayClasses: function (date) {
            ...
            if (date.isSame(today, 'month')) {  // <<=== monkey patch here (highlight current week)
                classes.push(
                    'fc-today',
                    view.highlightStateClass
                );
    ...
    var WeekGrid = DayGrid.extend({});
    WeekGrid = WeekGrid.extend(WeekTableMixin, {});

    fcViews.year = {
        'class': BasicView.extend({
            initialize: function () {
                BasicView.prototype.initialize.call(this);
                this.dayGrid = new WeekGrid(this);
            },
...
var ListViewGrid = Grid.extend({
	...
	dayHeaderHtml: function(dayDate) {
		...
		return '<tr class="fc-list-heading" data-date="' + dayDate.format('YYYY-MM-DD') + '">' +
			'<td class="' + view.widgetHeaderClass + '" colspan="4">' +
				(mainFormat ?
					view.buildGotoAnchorHtml(
						dayDate,
	...
	fgSegHtml: function(seg) {
		...
		if(event.attendees)
			attendeesHtml=this.fnGenerateAttendees(event);
	...
	/* generate html for attendees images */
	fnGenerateAttendees: function(event) {		
		var eventLength=event.attendees.length;
		if(eventLength > 0)
		{
			var attendeesPics='<span class="attendees-list">';
			for (i = 0; i < event.attendees.length; i++) 
			{
				var imgSrc=event.attendees[i].src;
				var title=event.attendees[i].name;
				attendeesPics+='<img title="'+title+'" data-placement="top" data-toggle="tooltip" data-original-title="'+title+'" class="multi-img" src="'+imgSrc+'">'; 
			}
			attendeesPics+='</span>';
			return attendeesPics;
		}
		else{
			return false;
		}	
